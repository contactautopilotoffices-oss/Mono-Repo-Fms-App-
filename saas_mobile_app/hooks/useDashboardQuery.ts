...
export async function fetchDashboardData(propertyId: string): Promise<DashboardData> {
  ...
  const bulkResults = await Promise.all([
    ...
    safeFetch(serverApi.query({ table: 'tickets', action: 'select', select: 'id', selectOptions: { count: 'exact', head: true }, filters: [propFilter, { op: 'in', column: 'status', values: ['resolved', 'closed', 'pending_validation'] }] }), null, 0),
    ...
    safeFetch(serverApi.query({ table: 'tickets', action: 'select', select: 'id', selectOptions: { count: 'exact', head: true }, filters: [propFilter, { op: 'gte', column: 'created_at', value: monthStr }, { op: 'in', column: 'status', values: ['resolved', 'closed', 'pending_validation'] }] }), null, 0),
    ...
    safeFetch(serverApi.query({ table: 'tickets', action: 'select', select: 'id', selectOptions: { count: 'exact', head: true }, filters: [propFilter, { op: 'gte', column: 'created_at', value: todayStr }, { op: 'in', column: 'status', values: ['resolved', 'closed', 'pending_validation'] }] }), null, 0),
