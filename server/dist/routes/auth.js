...
// Schemas
// ---------------------------------------------------------------------------
const TokenExchangeRequestSchema = zod_1.z.object({
    token: zod_1.z.string().min(1, 'Supabase JWT token is required'),
});
// Response schema for token exchange
const TokenExchangeResponseSchema = zod_1.z.object({
    token: zod_1.z.string(),
    expires_at: zod_1.z.number(),
    user_id: zod_1.z.string(),
    org_id: zod_1.z.string(),
    role: zod_1.z.string(),
    allowed_property_ids: zod_1.z.array(zod_1.z.string()),
    membership_id: zod_1.z.string().optional(),
});
...
async function verifySupabaseJWT(token, secret) {
    try {
        ...
        const payloadDecoded = Buffer.from(payloadB64.replace(/-/g, '+').replace(/_/g, '/') +
            '='.repeat((4 - (payloadB64.length % 4)) % 4), 'base64').toString('utf8');
...
const authRoutes = async (fastify) => {
    ...
    fastify.post('/auth/session', async (request, reply) => {
        ...
        if (isProduction) {
            ...
            fastify.log.info({ userId: payload.sub, orgId, role }, 'Token exchange successful (production)');
            return {
                token: sessionToken,
