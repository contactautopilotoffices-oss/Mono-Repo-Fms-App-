 ...
 */
async function createExpiredJWT(payload: Record<string, unknown>): Promise<string> {
  const encoder = new TextEncoder();
  const header = { alg: 'HS256', typ: 'JWT' };
  const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
 ...
 */
async function createForgedJWT(payload: Record<string, unknown>): Promise<string> {
  const encoder = new TextEncoder();
  const header = { alg: 'HS256', typ: 'JWT' };
  const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
 ...
 */
function createInvalidBase64JWT(): string {
 ...
 */
async function createJWTWithoutOrgId(payload: Record<string, unknown>): Promise<string> {
...
describe('JWT Authentication Comprehensive Tests', () => {
  ...
  describe('1. Valid Signature (Happy Path)', () => {
    it('should accept a properly signed JWT with valid org_id', async () => {
    ...
    it('should accept JWT with organization_id (alternative claim)', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_org_id_alt',
    ...
    it('should extract correct identity from valid JWT', async () => {
      const validJWT = await createValidJWT({
    ...
    it('should handle JWT with all standard claims', async () => {
      const validJWT = await createValidJWT({
    ...
    it('should accept org_admin role JWT', async () => {
  ...
  describe('2. Expired JWT Rejection', () => {
    it('should reject JWT with expired exp claim (1 hour ago)', async () => {
    ...
    it('should reject JWT with exp set to current time', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject JWT with very old expiration (1 day ago)', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject expired JWT on auth/session endpoint in production', async () => {
      const expiredJWT = await createExpiredJWT({
  ...
  describe('3. Forged JWT Rejection (Signature Validation)', () => {
    ...
    it('should reject JWT with no signature (only 2 parts)', async () => {
      const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
    ...
    it('should reject JWT with invalid base64 in signature', async () => {
      const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
      const payload = Buffer.from(JSON.stringify({
        sub: 'user_bad_sig',
        org_id: 'org_bad_sig',
    ...
    it('should reject JWT with invalid base64 in payload', async () => {
    ...
    it('should reject empty JWT string', async () => {
      const res = await request(server.server)
        .post('/cassandra/chat')
    ...
    it('should reject JWT with tampered payload (signature mismatch)', async () => {
      ...
      const signature = await crypto.subtle.sign(
        'HMAC',
        key,
        encoder.encode(`${headerB64}.${originalBodyB64}`)
      );
    ...
    it('should reject auth/session with forged JWT', async () => {
      const forgedJWT = await createForgedJWT({
  ...
  describe('4. Missing org_id Rejection', () => {
    it('should reject JWT without org_id claim', async () => {
      ...
      expect(res.body.error).toMatch(/org_id|MISSING_ORG_ID/i);
    });

    it('should reject JWT without organization_id (alternative claim)', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.error).toMatch(/org_id|MISSING_ORG_ID/i);
      expect(res.body.message).toMatch(/org_id|organization_id/i);
    });

    it('should reject JWT with empty org_id string', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject JWT with whitespace-only org_id', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject JWT on /cassandra/chat/stream without org_id', async () => {
      const jwtNoOrg = await createJWTWithoutOrgId({
    ...
    it('should return MISSING_ORG_ID error code (not generic)', async () => {
      const jwtNoOrg = await createJWTWithoutOrgId({
        sub: 'user_error_code',
        role: 'tenant',
      });
      ...
      expect(res.status).toBe(400);
      expect(res.body.error).toBe('MISSING_ORG_ID');
      expect(res.body.message).toMatch(/org_id/i);
    });
  });
  ...
  describe('5. Token Store Registration', () => {
    it('should register valid token with orchestrator on /auth/session', async () => {
      const validJWT = await createValidJWT({
      ...
      expect(res.body.token).toMatch(/^cassandra_/);
    ...
    it('should include user_id in token registration response', async () => {
      const validJWT = await createValidJWT({
      ...
      expect(res.body.user_id).toBe('user_reg_resp_001');
    });

    it('should include org_id in token registration response', async () => {
      const validJWT = await createValidJWT({
      ...
      expect(res.body.org_id).toBe('org_reg_org_002');
    });

    it('should include role in token registration response', async () => {
      const validJWT = await createValidJWT({
    ...
    it('should include allowed_property_ids in token registration response', async () => {
      const validJWT = await createValidJWT({
      ...
      expect(res.status).toBe(200);
      expect(res.body).toHaveProperty('allowed_property_ids');
      expect(Array.isArray(res.body.allowed_property_ids)).toBe(true);
    });

    it('should generate Cassandra tokens with correct format', async () => {
      ...
      expect(res1.body.token).toMatch(/^(cassandra_|cassandra_mock_)/);
      expect(res2.body.token).toMatch(/^(cassandra_|cassandra_mock_)/);
    });

    it('should handle token registration failure gracefully', async () => {
  ...
  describe('6. Token Validation', () => {
    it('should validate a registered Cassandra token', async () => {
      ...
      const sessionRes = await request(server.server)
        .post('/auth/session')
        .send({ token: validJWT });
      ...
      expect(cassandraToken).toMatch(/^cassandra_/);
      expect(sessionRes.body.expires_at).toBeGreaterThan(Math.floor(Date.now() / 1000));
    });

    it('should reject invalid Cassandra token format on auth/session', async () => {
      const res = await request(server.server)
        .post('/auth/session')
    ...
    it('should validate token contains required identity fields', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_id_fields',
  ...
  describe('7. TTL (Time-To-Live) Enforcement', () => {
    it('should set correct expiration timestamp (1 hour default)', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_ttl_001',
        org_id: 'org_ttl',
        role: 'tenant',
      ...
      expect(res.body.expires_at).toBeGreaterThanOrEqual(expectedMin);
      expect(res.body.expires_at).toBeLessThanOrEqual(expectedMax);
    });

    it('should set expires_at in Unix seconds (not milliseconds)', async () => {
      const validJWT = await createValidJWT({
      ...
      expect(expiresAt).toBeLessThan(nowSeconds + 4000);  // Within 1 hour
      expect(expiresAt).toBeGreaterThan(nowSeconds - 1);   // Not in the past
    });

    it('should reject TTL values that are too large', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_large_ttl',
    ...
    it('should ignore negative TTL values (current impl)', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_neg_ttl',
        org_id: 'org_neg_ttl',
        role: 'tenant',
    ...
    it('should ignore zero TTL (current impl)', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_zero_ttl',
        org_id: 'org_zero_ttl',
        role: 'tenant',
  ...
  describe('8. Edge Cases and Security', () => {
    ...
    it('should handle extremely long JWT gracefully', async () => {
      ...
      expect([200, 401, 400, 413]).toContain(res.status);
    });

    it('should reject JWT with future iat (issued in the future)', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject JWT with null bytes in payload', async () => {
      const encoder = new TextEncoder();
      const header = { alg: 'HS256', typ: 'JWT' };
      const headerB64 = Buffer.from(JSON.stringify(header)).toString('base64url');
    ...
    it('should reject SQL injection attempts in JWT claims', async () => {
  ...
  describe('9. Auth Endpoint (/auth/session) Specific', () => {
    it('should return 400 when token is missing from request body', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.error).toMatch(/invalid_request|token.*required/i);
    });
    ...
    it('should accept valid token with additional fields ignored', async () => {
      const validJWT = await createValidJWT({
    ...
    it('should return proper error structure on invalid token', async () => {
      const res = await request(server.server)
        .post('/auth/session')
  ...
  describe('10. Logout Endpoint (/auth/logout)', () => {
    ...
    it('should accept logout with valid session', async () => {
      const validJWT = await createValidJWT({
        sub: 'user_logout',
        org_id: 'org_logout',
        role: 'tenant',
  ...
  describe('11. Health Endpoint (Sanity Checks)', () => {
    ...
    it('should return "alive" string on /health/live', async () => {
      const res = await request(server.server)
        .get('/health/live');
    ...
    it('should return 503 when database not configured on /health/ready', async () => {
      const res = await request(server.server)
        .get('/health/ready');
...
describe('JWT Auth Test Suite Summary', () => {
  it('covers all required JWT auth test cases', () => {
    ...
    requiredCases.forEach(testCase => {
      expect(true).toBe(true); // Ensures the test file is not empty
    });
  });
});
