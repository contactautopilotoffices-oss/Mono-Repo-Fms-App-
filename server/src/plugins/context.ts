...
interface MembershipData {
  ...
  propertyMetadata: Record<string, { name: string; code: string; imageUrl?: string }>;
  cachedAt: number;      // Unix ms timestamp
  expiresAt: number;     // Unix ms timestamp (cachedAt + 24h)
}
...
const membershipCache = new Map<string, MembershipData>();
...
function getCachedMembership(userId: string, orgId: string): MembershipData | null {
...
function setCachedMembership(data: MembershipData): void {
  const key = cacheKey(data.userId, data.orgId);
  membershipCache.set(key, data);
}

function invalidateMembership(userId: string, orgId: string): void {
  const key = cacheKey(userId, orgId);
  membershipCache.delete(key);
}

function purgeAllMemberships(): void {
...
export const contextPlugin: FastifyPluginAsync = async (
  ...
  fastify.post('/context/hydrate', async (request, reply) => {
    ...
    try {
      ...
      const { data: propertyMemberships, error: propMemberErr } = await supabaseAdmin
      ...
      let propertyMetadata: Record<string, { name: string; code: string; imageUrl?: string }> = {};
      ...
      if (propertyMemberships && propertyMemberships.length > 0) {
        for (const pm of propertyMemberships) {
          if (pm.property) {
            const prop = pm.property as Record<string, unknown>;
            allowedPropertyIds.push(prop.id as string);
            propertyMetadata[prop.id as string] = {
              name: prop.name as string,
              code: prop.code as string,
              imageUrl: prop.image_url as string | undefined,
            };
          }
        }
      }
      ...
      if (ORG_ADMIN_ROLES.includes(userRole)) {
        const { data: orgProperties, error: orgPropsErr } = await supabaseAdmin
          .from('properties')
        ...
        if (orgPropsErr) {
          ...
          fastify.log.info(
            `[CONTEXT] Auto-injected ${orgProperties.length} org properties for org_admin user`
          );
        }
      }
      ...
      fastify.log.info(
        `[CONTEXT] Hydrated membership for user=${userId}, org=${orgId}, role=${userRole}, properties=${allowedPropertyIds.length}`
      );
  ...
  fastify.get('/context/status', async (request, reply) => {
    ...
    const orgId = (payload.org_id || payload.organization_id) as string;
    if (!userId || !orgId) {
      reply.status(400);
